...
Download or clone the virtual machine configuration from https://github.com/udacity/fullstack-nanodegree-vm into a fresh new directory and start it from there
downloas the data from https://d17h27t6h515a5.cloudfront.net/topher/2016/August/57b5f748_newsdata/newsdata.zip, unzip the folder and momve newsdata.sql file into the vagrant directory, which is shared with your virtual machine configuration.
...
create view artbyviews as select a.slug, count(*) as views from articles as a join log as l on a.slug = (regexp_split_to_array(path,E'/article/'))[2] where l.status = '200 OK' group by a.slug order by views desc;

create view errorview as select date(time) as day, count(*) as error from log where status= '404 NOT FOUND' group by day;

create view totalview as select date(time) as day, count(*) as total from log group by day ;
